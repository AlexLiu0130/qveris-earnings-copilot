...
- 当前缓存是服务端内存 TTL，适合 MVP 和本地协作；生产如果要永久分享页，需要接数据库或 KV。
...
默认 `EARNINGS_PROVIDER=qveris`，直接通过 QVeris 拉真实数据。当前后端会从本项目 `.env.local/.env` 以及相邻 `TradeMind_Agent/.env` 读取 `QVERIS_API_KEY`、`OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`。
